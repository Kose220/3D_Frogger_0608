...
//    SetUseLighting(true);
	...
	if (player.pos_z <= -600&& player.pos_x >= -15&&player.pos_x <= 15) {
		//effect_congra_play = PlayEffekseer3DEffect(effect_congra);
...
{   //  道路の作成（車込み）
	...
	if (title) {
		...
		DrawRotaGraph(Title_Player_x - Title_Slide_x, WINDOW_H - 80, 0.5f, 0, Title_Player, true);
		DrawRotaGraph(300 - Title_Slide_x, WINDOW_H - 200, 0.7f, 0, Title_House, true);

		DrawRotaGraph(Title_Ufo_x - Title_Slide_x, Title_Ufo_y, 0.25f, 0, Title_Ufo, true);
